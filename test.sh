...
TEST_RED="$(printf '\e[31m')"
TEST_GREEN="$(printf '\e[32m')"
TEST_YELLOW="$(printf '\e[33m')"
TEST_RESET="$(printf '\e[0m')"

# Param: command expected_output [exit_code]
test () {
    local actual_output
    echo -n "Testing '${TEST_YELLOW}$1${TEST_RESET}'... "
    ...
    if [ "$2" = "$actual_output" -a '(' -z "$3" -o "$3" = "$?" ')' ]; then
        ...
        echo -e "\tExpected '$2' got '$actual_output' instead."
        total_tests=$((total_tests + 1))
        return 1
    fi
}
...
# BEGIN_USERS_TESTS
. src/map.sh
map-set test_map foo bar
test 'map-get test_map foo' 'bar'
map-set test_map fooz 'bar baz'
test 'map-get test_map fooz' 'bar baz'
...
test 'list-get 0 $list' '1'
test 'list-get 4 $list' '5'
test 'list-get 7 $list' '8'
test 'list-get -1 $list' '' '1'
test 'list-get 8 $list' '' '1'
test 'list-set -1 0 $list' '1 2 3 4 5 6 7 8' '1'
test 'list-set 8 0 $list' '1 2 3 4 5 6 7 8' '1'
test 'list-set 1 23 $list' '1 23 3 4 5 6 7 8'
test 'list-set 7 23 $list' '1 2 3 4 5 6 7 23'
list="a 'b c' d"
